...
class ContextAttention(nn.Module):
    def __init__(
        ...
        assert num_prototypes == dim
        self.dim = dim
        self.n_h = num_heads
        self.h_d = dim // num_heads
        self.n_proto = num_prototypes
        
        self.x_d_to_dm = nn.Linear(dim, num_prototypes + dim, bias=qkv_bias)
        self.A_n_to_n = nn.Parameter(torch.zeros(self.n_h, num_tokens, num_tokens))
        self.A_m_to_n = nn.Parameter(torch.zeros(self.n_h, self.h_d, num_tokens))
        self.out_d_to_d = nn.Linear(dim, dim, bias=proj_bias)
        
        with torch.no_grad():
            trunc_normal_(self.A_n_to_n, std=0.02)
            trunc_normal_(self.A_m_to_n, std=0.02)
        
    def forward(self, x):
        B, N, D = x.shape 
        H, d, M = self.n_h, self.h_d, self.n_proto

        # B, N, 4D = A.shape
        x_q, x_v = self.x_d_to_dm(x).view(B, N, 2, H, d).permute(2, 0, 3, 1, 4) # 2[B, H, N, d]
        x_q = x_q.transpose(-1, -2) # [B, H, d, N]
        A_m_n =  torch.matmul(x_q, self.A_n_to_n) # B, H, M, N
        
        A_n_m = A_m_n.transpose(-1, -2) # [B, H, N, M]
        A_n_n = torch.matmul(A_n_m, self.A_m_to_n) # B, H, N, N
        
        x_attn = F.softmax(A_n_n, -1) @ x_v # [B, H, N, d]
        x_out = x_attn.transpose(1, 2).reshape(B, N, D) # [B, N, D]
        return self.out_d_to_d(x_out) # [B, N, D]

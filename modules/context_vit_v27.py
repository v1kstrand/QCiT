...
class ContextAttention(nn.Module):
    def __init__(
        ...
        self.A_kd_to_n = nn.Linear(self.A_d, num_tokens, bias=False)
        self.out_d_to_d = nn.Linear(dim, dim, bias=proj_bias)
        
    def forward(self, x, A):
        B, N, D = x.shape 
        H, d, A_d = self.n_h, self.h_d, self.A_d

        # B, N, 4D = A.shape
        x_v = self.x_d_to_d(x) #[B, N, D]
        A_kd_n =  self.A_n_to_n(A.transpose(1, 2)) # B, 4D, N
        A_n_n =  self.A_kd_to_n(A_kd_n.transpose(1, 2)) # B, N, N
        x_w = F.softmax(A_n_n, -1) @ x_v
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        def attn_residual_func(x: Tensor) -> Tensor:
            return self.ls1(self.attn(self.norm1(x), self.mlp.get_A()))

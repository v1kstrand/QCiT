...
class CPBtable(nn.Module):
    def __init__(self, N, R, H):
        super().__init__()
        self.make_cpb_pos_tables(N,R,H)
    ...
    def make_cpb_pos_tables(self,N,R,H):
        ...
        self.register_buffer("rel", rel_table, persistent=False)  # [L,2]
        ...
        d[:, :, 1] += S - 1
        d[:, :, 0] *= 2 * S - 1
        l_idx = d.sum(-1).to(torch.int32)  # [P,P] in [0..L-1]

        idx = torch.full((N, N), 0, dtype=torch.int32)  
        idx[R:, R:] = l_idx 
        self.register_buffer("idx", idx, persistent=False)  # [N,N], {-1 U [0..L-1]}

...
class ContextAttention(nn.Module):
    def __init__(
        ...
        self.bank = nn.Parameter(torch.randn(1, self.M, self.K, ckw["num_tokens"]))
        self.cls_to_w = nn.Linear(dim, self.M)
    ...
    def forward(self, x):
        ...
        bank       = self.bank.expand(B, -1, -1, -1)               # [B, M, K, N]
        w_bank     = F.softmax(self.cls_to_w(x[:, 0, :]), -1)                   # [B, M]
        w_ctx      = (w_bank.view(B, M, 1, 1) * bank).sum(dim=1)  # [B, K, N]
        ctx        = F.softmax(w_ctx, -1) @ x      # [B, K, D]             

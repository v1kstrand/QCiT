...
class CPB2D(nn.Module):
    ...
    def forward(self, dtype: torch.dtype) -> torch.Tensor:
        ...
        K_ctx_pos = (self.tile_centers + self.u_pos).reshape(1, K_ctx, 2 + self.S)

        # --- relative features for patch→ctx block
        delta = (self.P_pos - K_ctx_pos)   # [P, K_ctx, 3]
        delta_xy, s_feat = torch.split(delta, (2, self.S), -1)            # [P, K_ctx, 2]
        phi_xy   = torch.sign(delta_xy) * torch.log1p(delta_xy.abs())
        feats_ctx = torch.cat([phi_xy, -s_feat], dim=-1)          # [P, K_ctx, 3]
        out_ctx = self.mlp(feats_ctx.to(dtype))  # [P, K_ctx, H]# [P, K_ctx, 3]

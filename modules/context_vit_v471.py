...
class ContextAttention(nn.Module):
    def __init__(
        ...
        self.Q_ctx = nn.Parameter(torch.randn(self.K - self.R, self.N))
        self.K_ctx = nn.Parameter(torch.randn(self.K - self.R, self.M, self.d))
    ...
    def forward(self, x):
        ...
        return       self.out_drop(self.proj_out(y)), (pi.detach(), w_ctx.detach(), x_ctx.detach()) # [B, N, D] - cache
...
class ContextViTv471(nn.Module):
    def __init__(
        self,
        ckw,
        img_size=224,

...
class ContextAttention(nn.Module):
    def __init__(
        ...
        self.x_d_to_dm = nn.Linear(dim, dim + num_prototypes, bias=qkv_bias)
        self.A_n_to_n = nn.Linear(num_tokens, num_tokens, bias=proj_bias)
        self.A_m_to_n = nn.Linear(num_prototypes, num_tokens, bias=proj_bias)
        self.out_d_to_d = nn.Linear(dim, dim, bias=proj_bias)
        
    def forward(self, x):
        B, N, D = x.shape 
        x_q, x_v = torch.split(self.x_d_to_dm(x), (self.n_proto, D), -1) #2[B, N, M/D]
        A_kd_n =  self.A_n_to_n(x_q.transpose(1, 2)) # [B, M, N]
        A_n_n =  self.A_m_to_n(A_kd_n.transpose(1, 2)) # [B, N, N]
        x_w = F.softmax(A_n_n, -1) @ x_v # [B, N, D]
        return self.out_d_to_d(x_w) # [B, N, D]

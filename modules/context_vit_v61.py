...
class ContextAttention(nn.Module):
    """
    U-way window pooling per tile (td×td, non-overlapping; stride == td) -> global cross-attention.
    Works for any tile_dim by padding right/bottom so (Se % td == 0).

    Notes
    -----
    - P = num_tokens - num_regs must be a perfect square (S×S).
    - We pad logits with -inf (so softmax zeroes padded cells) and features with 0.
    - Complexity: SDPA over K = R + (ceil_div(S, td))^2 * U.
    """
    ...
    def __init__(
        ...
        proj_drop: float = 0.0,
        proj_bias: bool = True,
        tile_comp_size: int = 5,   # U
        tile_dim: int = 2,         # td (window side)
    ):
        super().__init__()
        ...
        assert S * S == self.P, f"patch count P ({self.P}) must be a square"
        self.S  = S

        self.td = int(tile_dim)
        self.ts = self.td * self.td
        self.U  = tile_comp_size if isinstance(tile_comp_size, int) else int(self.ts * tile_comp_size)
        self.stride = self.td

        # 1x1 projection to U logits per patch
        self.logit = nn.Linear(dim, self.U, bias=False)
        self.logit.no_wd = True  # optional flag for your optimizer grouping
    ...
    def forward(self, x: torch.Tensor):
        ...
        U, S, td, ts = self.U, self.S, self.td, self.ts

        # Split registers / patches
        regs  = x[:, :R, :]         # [B,R,D]
        patch = x[:, R:, :]         # [B,P,D], P=S*S

        # (1) logits on flat grid -> [B,U,S,S]
        logsU_flat = self.logit(patch)                               # [B,P,U]
        logsU_img  = logsU_flat.transpose(1, 2).reshape(B, U, S, S)  # [B,U,S,S]
        ...
        if pad_side:
            logsU_img = F.pad(logsU_img, (0, pad_side, 0, pad_side), value=float("-inf"))  # [B,U,Se,Se]

        # (2) view-based windowing for logits (stride == td)
        lg = logsU_img.unfold(2, td, td).unfold(3, td, td)                 # [B,U,Th,Tw,td,td]
        Th, Tw = lg.size(2), lg.size(3)
        T = Th * Tw
        win_logits = lg.permute(0, 2, 3, 1, 4, 5).reshape(B, T, U, ts)     # [B,T,U,ts]

        # (3) softmax over window positions (fp32 for stability)
        w = F.softmax(win_logits.float(), dim=-1).to(patch.dtype)          # [B,T,U,ts]

        # (4) gather features once and pool with w
        x_img = patch.transpose(1, 2).reshape(B, D, S, S)                  # [B,D,S,S]
        if pad_side:
            x_img = F.pad(x_img, (0, pad_side, 0, pad_side), value=0.0)    # [B,D,Se,Se]

        tiles_view = x_img.unfold(2, td, td).unfold(3, td, td)             # [B,D,Th,Tw,td,td]
        tiles = tiles_view.permute(0, 2, 3, 4, 5, 1).reshape(B, T, ts, D)  # [B,T,ts,D]

        # Batched GEMM pooling: [B*T,U,ts] @ [B*T,ts,D] -> [B,T,U,D]
        ctx_win = (w.reshape(B*T, U, ts) @ tiles.reshape(B*T, ts, D)).view(B, T, U, D)
        ctx_learn = ctx_win.reshape(B, T*U, D)                             # [B,T*U,D]

        # Prepend registers
        ctx = torch.cat([regs, ctx_learn], dim=1)                          # [B,K,D]
        K = ctx.size(1)

        # (5) global attention to compressed K/V
        q  = self.proj_q(x).view(B, N, H, d).transpose(1, 2)               # [B,H,N,d]
        kv = self.proj_kv(ctx).view(B, K, 2, H, d).permute(2, 0, 3, 1, 4)
        k, v = torch.unbind(kv, 0)                                         # [B,H,K,d]
...
class ContextViTv61(nn.Module):
    ...
    def init_weights(self):
        ...
        P, td, tcs = self.n_patches, self.ckw["tile_dim"], self.ckw["tile_comp_size"]
        U  = tcs if isinstance(tcs, int) else int((td ** 2) * tcs)
        print(f"INFO - ContextViTv61: U: {U}")
    ...
    def make_cpb_pos_tables(
        self,
        P: int,              # patch count (P = S*S)
        td: int,             # tile side (td x td), stride == td
        U: int,              # pooled tokens per tile
        init_u_offset: float = 0.0,  # optional small init for Δcx,Δcy
        init_u_spread: float = 0.0,  # initial log-spread s
    ):
        """
        Returns
        -------
        P_pos        : [P, 1, 3] with [qx, qy, 0]  (coords on original SxS grid in [-1,1])
        tile_centers : [T, 1, 3] with [cx, cy, 0]  (one per padded td×td window; stride=td)
        u_pos        : [1, U, 3] with [Δcx, Δcy, s], shared across windows
        """
        S = int(P ** 0.5)
        assert S * S == P and td > 0 and U > 0

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        # --- Patch coords on original SxS grid in [-1, 1]
        ...
        P_pos = torch.zeros(P, 1, 3, device=device)
        P_pos[:, 0, 0] = Qx
        P_pos[:, 0, 1] = Qy
        # last channel stays 0

        # --- Pad right/bottom so Se % td == 0 (non-overlap windows fit perfectly)
        pad_side = (td - (S % td)) % td
        Se = S + pad_side

        # Valid mask on Se×Se (1 inside S, 0 in pad)
        valid = torch.zeros((Se, Se), device=device)
        valid[:S, :S] = 1.0

        # Pad coords with zeros (they'll be ignored by the mask)
        GXp = torch.zeros((Se, Se), device=device); GXp[:S, :S] = GX
        GYp = torch.zeros((Se, Se), device=device); GYp[:S, :S] = GY

        # --- Non-overlap windows via view-based unfold (kernel=td, stride=td)
        # shapes: [Th, Tw, td, td]
        Gx_w = GXp.unfold(0, td, td).unfold(1, td, td)
        Gy_w = GYp.unfold(0, td, td).unfold(1, td, td)
        M_w  =  valid.unfold(0, td, td).unfold(1, td, td)

        Th, Tw = Gx_w.size(0), Gx_w.size(1)
        T = Th * Tw

        # Weighted mean over valid cells in each window
        denom = M_w.sum(dim=(-1, -2), keepdim=True).clamp_min_(1.0)    # [Th,Tw,1,1]
        Cx = (Gx_w * M_w).sum(dim=(-1, -2)) / denom.squeeze(-1).squeeze(-1)  # [Th,Tw]
        Cy = (Gy_w * M_w).sum(dim=(-1, -2)) / denom.squeeze(-1).squeeze(-1)  # [Th,Tw]

        tile_centers = torch.zeros((T, 1, 3), device=device)
        tile_centers[:, 0, 0] = Cx.reshape(-1)
        tile_centers[:, 0, 1] = Cy.reshape(-1)
        # last channel stays 0

        # --- Per-U learnable anchor (Δcx, Δcy, s), shared across windows
        u_pos = torch.zeros((1, U, 3), device=device)
        if init_u_offset:
            u_pos[:, :, 0:2].fill_(init_u_offset)
        if init_u_spread:
            u_pos[:, :, 2].fill_(init_u_spread)

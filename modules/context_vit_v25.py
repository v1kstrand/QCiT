...
class ContextAttention(nn.Module):
    def __init__(
        ...
        if use_bias:
            self.N_to_D_bias = nn.Parameter(torch.zeros(1, self.n_h, 1, self.h_d)) 
            self.M_to_N_bias = nn.Parameter(torch.zeros(1, self.n_h, 1, num_tokens))
        else:
    ...
    def forward(self, x):
        B, N, D = x.size()
        H, d, M = self.n_h, self.h_d, self.num_prototypes

        # Prepare heads: [B,H,N,d]
        x_heads = x.view(B, N, H, d).transpose(1, 2)

        # Prototypes projection: [B,H,N,d] → [B,H,N,M]
        A = torch.matmul(x_heads, self.prototypes_weight)  # [H,d,M] 
        ...
        A_dn = torch.matmul(A_md_trans, self.M_to_N_weight) + self.M_to_N_bias

        # Reshape to original dimensions: [B,H,d,N] → [B,N,D]
        x_out = A_dn.permute(0, 2, 1, 3).reshape(B, N, D)

        # Merge heads explicitly
        return self.head_merge(x_out)

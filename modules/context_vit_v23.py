...
class ContextAttentionMultiHead(nn.Module):
    def __init__(
        ...
        self.prototypes = nn.Linear(self.n_h, self.h_d, num_prototypes)
    ...
    def forward(self, x):
        ...
        x = self.proj(x).view(B, N, H, d) # 
        score_MN = torch.matmul(x, self.prototypes) 
